...
/**
 * Get a fast/small Groq LLM instance for grading and query rewriting (free tier).
 * Uses temperature 0 for deterministic evaluation.
 */
export function getGraderLlm() {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
    throw new Error('GROQ_API_KEY environment variable is not set');
  }

  const model = process.env.GROQ_GRADER_MODEL || GRADER_MODEL;
...
3. If the context does not contain enough information to answer the question, say: "Your notes don't seem to cover this topic. You can switch to Agent mode for a broader search."
...
export function buildAgentSystemPrompt(contextBlock: string): string {
  return `You are Chalk, a study assistant. In Agent mode, you use both the student's notes and external knowledge (like web search results or your own broad knowledge) to answer questions.

## CRITICAL RULES — AGENT MODE
...
export const buildExploreSystemPrompt = buildAgentSystemPrompt;
